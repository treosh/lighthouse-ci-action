    ...
    createMethodInfoLiteral(source, methodInfo) {
        ...
        for (let key of ["name", "localName", "idempotency", "serverStreaming", "clientStreaming", "options"]) {
            if (methodInfo[key] !== undefined) {
                properties.push(ts.createPropertyAssignment(key, typescript_literal_from_value_1.typescriptLiteralFromValue(methodInfo[key])));
            }
        }
        // I: The generated type handler for the input message.
        properties.push(ts.createPropertyAssignment(ts.createIdentifier('I'), ts.createIdentifier(this.imports.typeByName(source, methodInfo.I.typeName))));
        // O: The generated type handler for the output message.
        properties.push(ts.createPropertyAssignment(ts.createIdentifier('O'), ts.createIdentifier(this.imports.typeByName(source, methodInfo.O.typeName))));
