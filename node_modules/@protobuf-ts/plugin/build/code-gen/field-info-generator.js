...
class FieldInfoGenerator {
    ...
    createFieldInfoLiteral(source, fieldInfo) {
        ...
        for (let key of ["no", "name", "kind", "localName", "jsonName", "oneof"]) {
            if (fieldInfo[key] !== undefined) {
                properties.push(ts.createPropertyAssignment(key, typescript_literal_from_value_1.typescriptLiteralFromValue(fieldInfo[key])));
            }
        }
        ...
        if (fieldInfo.opt !== undefined) {
            properties.push(ts.createPropertyAssignment("opt", typescript_literal_from_value_1.typescriptLiteralFromValue(fieldInfo.opt)));
        }
        ...
        if (fieldInfo.options) {
            properties.push(ts.createPropertyAssignment(ts.createIdentifier('options'), typescript_literal_from_value_1.typescriptLiteralFromValue(fieldInfo.options)));
        }
    ...
    createEnumT(source, ei) {
        ...
        runtime_1.assert(descriptor);
        let generatedEnum = this.imports.type(source, descriptor), enumInfoLiteral = [
