...
class InternalBinaryWrite {
    ...
    makeMethod(source, descMessage, bodyStatements) {
        const MessageInterface = this.imports.type(source, descMessage), IBinaryWriter = this.imports.name(source, 'IBinaryWriter', this.options.runtimeImportPath, true), BinaryWriteOptions = this.imports.name(source, 'BinaryWriteOptions', this.options.runtimeImportPath, true);
    ...
    makeStatementsForEveryField(source, descMessage) {
        const interpreterType = this.interpreter.getMessageType(descMessage), statements = [];
        for (let fieldInfo of interpreterType.fields.concat().sort((a, b) => a.no - b.no)) {
            ...
            let fieldDeclarationComment = protoplugin_1.getDeclarationString(descField) + ";";
    ...
    scalar(source, field, fieldPropertyAccess, fieldDeclarationComment) {
        ...
        if (field.T === rt.ScalarType.BYTES && field.opt) {
            ...
            let defaultValueExpression = typescript_literal_from_value_1.typescriptLiteralFromValue(defaultValue);
    ...
    message(source, field, fieldPropertyAccess, fieldDeclarationComment) {
        ...
        let binaryWrite = ts.createCall(ts.createPropertyAccess(ts.createIdentifier(this.imports.typeByName(source, field.T().typeName)), ts.createIdentifier("internalBinaryWrite")), undefined, [fieldPropertyAccess, writeTagAndFork, ts.createIdentifier("options")]);
        // <...>.join()
    ...
    messageRepeated(source, field, fieldPropertyAccess, fieldDeclarationComment) {
        ...
        let binaryWrite = ts.createCall(ts.createPropertyAccess(ts.createIdentifier(this.imports.typeByName(source, field.T().typeName)), ts.createIdentifier("internalBinaryWrite")), undefined, [fieldPropI, writeTagAndFork, ts.createIdentifier("options")]);
        // <...>.join()
        let binaryWriteAndJoin = this.makeWriterCall(binaryWrite, 'join');
    ...
    messageOneof(source, field, fieldDeclarationComment) {
        ...
        let binaryWrite = ts.createCall(ts.createPropertyAccess(ts.createIdentifier(this.imports.typeByName(source, field.T().typeName)), ts.createIdentifier("internalBinaryWrite")), undefined, [
    ...
    map(source, field, fieldPropertyAccess, fieldDeclarationComment) {
        ...
        if (field.V.kind == "message") {
            ...
            forBody = ts.createBlock([
                ...
                ts.createExpressionStatement(ts.createCall(ts.createPropertyAccess(ts.createIdentifier(this.imports.type(source, descMessage)), ts.createIdentifier("internalBinaryWrite")), undefined, [
                    mapEntryValueRead,
                    ts.createIdentifier("writer"),
                    ts.createIdentifier("options")
                ])),
    ...
    makeWriterCall(writerExpressionOrName, type, argument) {
        let methodName = typeof type == "string" ? type : protobuf_1.ScalarType[type].toLowerCase();
